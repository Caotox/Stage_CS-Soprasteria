...
def isGlobalSpecificConf(d):
    if len(d) == 1 and ('__Global__' in d or '__Specifique__' in d) or (
            len(d) == 2 and ('__Global__' in d and '__Specifique__' in d)):
        return True
    else:
        return False
...
def lireTabFichiers(tabFichier, configFileDir=None, libelle='-'):
    ...
    if tabFichier[0] == "XLSX":
        if len(tabFichier) == 6:
            ...
            logging.info(
                "fichier={} feuilles={} transpose={} nbColonnesAIgnorer={} nbLignesAIgnorer={} ".format(fichier,
                                                                                                        feuilles,
                                                                                                        transpose,
                                                                                                        nbColonnesAIgnorer,
                                                                                                        nbLignesAIgnorer))
            ...
            for feuille in feuilles:
                newListeItems = convertExeclToDict(fileName=fichier, sheetName=feuille, transpose=transpose,
                                                   nbColonnesAIgnorer=nbColonnesAIgnorer,
                                                   nbLignesAIgnorer=nbLignesAIgnorer)
                listeItems.update(newListeItems)
        else:
            logging.error(
                "lireTabFichiers() {} contient {} éléments : il en faut 6".format(tabFichier, len(tabFichier)))
    else:
                ...
                if fichier.suffix in [".json", ".jsn", '.yaml', '.yml']:
                    ...
                    if not newItems:
                        ...
                        if isinstance(newItems, list):
                            ...
                            logging.error(
                                f"lireTabFichiers() : libelle=[{libelle}] fichier={fichier} type inconnu={type(newItems)}")
                    else:
                        # 1er tour de boucle
                        listeItems = newItems
                else:
                    logging.error(
                        f"lireTabFichiers() : fichier={fichier} -> libelle=[{libelle}] extension inconnue=[{fichier.suffix}]")
    return listeItems
...
def dataToInclude(contenuCleInclure: str | list | dict, configFileDir: str = None) -> dict:
    ...
    if isinstance(contenuCleInclure, str):
        ...
        logging.error(
            "dataToInclude() le contenu de la clé {} n'est pas exploitable type={} contenu={}".format('__Inclure__',
                                                                                                      type(
                                                                                                          contenuCleInclure),
                                                                                                      contenuCleInclure))
    ...
    if fichierAInclure != None and cleAInclure != None:
        ...
        logging.error("dataToInclude() pb avec la clé contenuCleInclure={} fichierAInclure={} cleAInclure={}".format(
            contenuCleInclure, fichierAInclure, cleAInclure))
    return data


def gestionInclude(data, configFileDir=None,cle=True):
    ...
    if type(data) is dict:
        while cle :
            ...
            if '__Inclure__' in data:
                ...
                cle = '__include__'
            elif '__Include3__' in data:
                cle = '__Include3__'
            """
            for n in range(101):
                if "__Inclure"+str(n)+"__" in data:
                    cle = "__Inclure"+str(n)+"__"
                    break
                else:
                    cle = None
            if cle:
                ...
                if originalData == data:
                    pass
                elif originalData != data: # Si les deux dicos sont différents
                    for n, v in data.items(): # On parcours data
                        if type(n) == dict or type(v) == dict: # Si une de ses clés ou valeurs est un dico on va le parcourir
                            for i in data[n]: # data[n] va directement chercher la valeur de data[n], donc le dico et on va le parcourir
                                if n in originalData.keys(): # On va voir si la clé associée à un dico dans data se trouve aussi dans originalData, donc si les deux dicos ont un dictionnaire "semblable"
                                    if i not in originalData[n]: #  Si i (valeur du parcours du dico de data) ne se trouve pas dans original data, on ajoute la clé et la veur à une liste
                                        temp.append(n) #temp va permettre ensuite d'ajouter manuellement les clés ainsi récupérées au dico afin que l'update permette de garder les valeurs supplémentaires
                                        temp.append(v)
                for j in range(len(temp) // 2):
                    if len(temp) == 0:
                        ...
                        pass
                    elif str(data[temp[0]]) not in originalData: # Si la clé récupérée n'est pas dans original data
                        for p in temp[1 + ecart].keys(): # On parcours les clés du dico (donc de la valeur récupérée précedemment)
                            if p in originalData[temp[0 + ecart]].keys(): # Si originalData possède également une clé en commun
                                originalData[temp[0 + ecart]][p] = originalData[temp[0 + ecart]][p] # On update originalData avec le coup
...
def main(name_fichier):
    """
    :param name_fichier: nom du fichier à traiter
    :return: un affichage du fichier yaml (dictionnaire) mis à jour

    C'est la fonction à appeler lors de l'import du programme en tant que module
    Soit :
    import dicos
    dicos.main(fichier.yaml)

    Ou encore
    from dicos import main
    main(fichier.yaml)
    """
    ...
    #confMngr = configItemsGenerique(fichier='essai.yaml')
    confMngr = configItemsGenerique(fichier=name_fichier)
    if confMngr:
